...
module.exports = {
  ...
  try {
    ...
    const product = ticket?.product_id
      ? config.products.find((p) => p.id === ticket.product_id)
      : null;
      ...
      return interaction.editReply({
        embeds: [
          dangerEmbed(
            ...
            "Não foi possível identificar o produto deste carrinho."
          )
        ]
      });
    }
    ...
    console.log("[DEBUG] Enviando menu de gateway");
    ...
    await interaction.editReply({
      embeds: [
        infoEmbed(
          ...
          `Escolha o gateway de pagamento para **${product.name}**.\n\n**Total:** ${formatPrice(product.price)}`
        )
      ],
      components: [gatewayRow]
    });
    ...
    console.error("[DEBUG] Erro no handler:", error);
