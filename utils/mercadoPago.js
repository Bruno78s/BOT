...
async function attachProviderPaymentIdByReference(channelId, providerPaymentId, status) {
  console.log(`[MERCADO_PAGO] Atualizando pagamento para channelId=${channelId}, providerPaymentId=${providerPaymentId}`);
  await run(
    "UPDATE payments SET provider_payment_id = ?, updated_at = ? WHERE channel_id = ? AND status = 'pending'",
